...
// Pure-JS SHA-256 — no crypto.subtle, works in any context (HTTP, IP, etc.)
function sha256(msg: Uint8Array): Uint8Array {
  ...
  const len = msg.length, padLen = (len % 64 < 56 ? 56 : 120) - (len % 64)
  const data = new Uint8Array(len + padLen + 8)
  data.set(msg); data[len] = 0x80
  const dv = new DataView(data.buffer)
  dv.setUint32(data.length - 4, len * 8, false)
...
function persist(access: string, refresh: string, expiresIn: number): void {
  const t: Tokens = { access, refresh, expiresAt: Date.now() + (expiresIn - 60) * 1000 }
