...
import {
  ...
  startPlayback,
  transferPlayback,
} from "./features/spotify/playerApi";
    ...
    loadSdk()
      .then(() => {
        ...
        player.addListener("ready", ({ device_id }) => {
          ...
          void transferPlayback(device_id).catch((e) =>
            onError?.(
              `Spotify transfer failed: ${e instanceof Error ? e.message : String(e)}`,
            ),
          );
          ...
          const playUris = async (
            ...
            await startPlayback(
              { uris, offset: { position: Math.max(0, offsetIndex) } },
              device_id,
            );
          };
